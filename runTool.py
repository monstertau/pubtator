...
def mergeBiocWithMetadata(metaDir, inDir, outBioc):
    filenames = sorted([filename for filename in os.listdir(inDir) if
                        filename.lower().endswith('.xml') and not filename.lower().endswith('.ga.xml')])
...
def mergeBioc(inDir, outBioc):
    inBiocs = sorted([os.path.join(inDir, filename) for filename in os.listdir(inDir) if
                      filename.lower().endswith('.xml') and not filename.lower().endswith('.ga.xml')])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--maxLength', type=int, default=1000000,
                        help='Max size (in characters) of documents to put in a single file for processing')
    parser.add_argument('--debug', action='store_true',
                        help='Whether to use the "temp" directory and not delete intermediate files')
    ...
    size = os.path.getsize(inBioc)
    if size == 0:
        sys.exit()
    here = os.path.abspath(os.path.dirname(__file__))

    toolDirs = {'dnorm': 'DNorm-0.0.7', 'gnormplus_java': 'GNormPlusJava', 'gnormplus_perl': 'GNormPlus',
                'tmchem': 'tmChemM1-0.0.2', 'tmvar': 'tmVar3'}
    ...
    with TempDir(args.debug) as tempDir:
        ...
        if tool == 'dnorm':
            ...
            command = ['sh', 'ApplyDNorm_BioC.sh', 'config/banner_NCBIDisease_UMLS2013AA_TEST.xml',
                       'data/CTD_diseases.tsv', 'output/simmatrix_NCBIDisease_e4.bin', ab3pDir,
                       os.path.join(workingDir, 'tmp'), inputDir, outputDir]
        elif tool == 'gnormplus_java':
            jarFile = os.path.join(toolDir, 'GNormPlus.jar')
            setupFile = os.path.join(toolDir, 'setup.9606.txt')
            ...
            ab3pDir = os.path.join(here, 'Ab3P-v1.5')
            command = ['sh', 'Run.sh', 'config/banner_JOINT.xml', 'data/dict.txt', ab3pDir,
                       os.path.join(workingDir, 'tmp'), inputDir, outputDir, str(args.mem)]
        elif tool == 'tmvar':
